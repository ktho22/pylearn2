...
class H5Shuffle(Dataset):
	...
	if _iter_num_batches is None:
        ...
        def getFeatures(indexes):
            ...
            replace_idx = numpy.random.randint(0, self.frame_length, len(X))
            replaced_word_idx = numpy.random.randint(0,self.X_labels, len(X))
            X[numpy.arange(len(X)),replace_idx] = replaced_word_idx

            y = numpy.zeros((len(X), self.frame_length))
